...
const handleDiagram = (sessionId: string, payload: string) => {
  ...
  const diagramData = {
    nodes: [
      { id: '1', type: 'input', data: { label: `Input: "${payload}"` }, position: { x: 250, y: 5 } },
      { id: '2', data: { label: 'AI xử lý...' }, position: { x: 250, y: 100 } },
    ],
    edges: [
      { id: 'e1-2', source: '1', target: '2', type: 'custom', markerEnd: { type: 'arrowclosed' } },
    ],
  };
...
const handleImprove = (sessionId: string) => {
  ...
  if (!sessions[sessionId] || !sessions[sessionId].diagram) {
    return {
      action: 'DISPLAY_TEXT',
      message: 'Lỗi: Không tìm thấy sơ đồ nào trong phiên làm việc này để phân tích. Hãy dùng @diagram trước.'
    };
  }
...
// API chính để chat
export const mockApiCall = (tag: string, payload: string, sessionId: string): Promise<any> => {
    ...
    setTimeout(() => {
      switch (tag) {
        ...
        case '@improve':
          ...
          break;
        case '@ask':
          resolve({ action: 'DISPLAY_TEXT', message: `Đã nhận câu hỏi cho session ${sessionId}.` });
          break;
        default:
          resolve({ action: 'DISPLAY_TEXT', message: "Lệnh không hợp lệ. Hãy dùng @diagram, @ask, @improve." });
          break;
      }
    }, 1000);
  });
...
export const getSessionData = (sessionId: string, dataType: 'diagram' | 'analysis'): Promise<any> => {
    ...
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            if (sessions[sessionId] && sessions[sessionId][dataType]) {
                ...
                reject(new Error(`Không tìm thấy dữ liệu '${dataType}' cho session ID này.`));
            }
        }, 500);
    });
};